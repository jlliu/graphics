...
let tree = createObjectVertices(tree_mesh);
let star = createObjectVertices(star_mesh);

let apple = createObjectVertices(apple_mesh);
...
// A list of the x,y,z coordinates for each apple
var apples = {};

var anAppleHasBeenPicked = false;

var bg = [1,1,1];


let isOverObj = (tipPos, objPos) =>{
    let x = objPos[0] - tipPos[0];
   let y = objPos[1] - tipPos[1];
   let z = objPos[2] - tipPos[2];
   if (Math.sqrt(x**2+y**2+z**2) < .08){
    return true;
   } else {
    return false;
   }
...
var current_RC_position = [];

var currentlyMovingAnApple = false;
function onStartFrame(t, state) {
    ...
    if (RC){
      ...
      current_RC_position = RC.tip().slice();

      //Check if apples list is populated
      if ( apples!== {} ){
          for (var i = 0; i < Object.keys(apples).length; i++){
            var a = apples[i];
            if (a["isPicked"] == false){
              var apple_matrix = a["originalPos"];
              var apple_translation = [apple_matrix[12],apple_matrix[13],apple_matrix[14]];
            }
            else if  (a["isMoving"]) {
                // var apple_matrix = current_RC_position;
                var apple_translation = current_RC_position;
            }
            else {
               var apple_translation = a["newPos"];
            }
            var tip = RC.tip();
            //Give feedback if we are hovering over apple
            if (isOverObj(tip, apple_translation)) {
              a["hover"] = true;
            } else {
              a["hover"] = false;
            }
            // If we are grabbing apple: controller is down and it's over object
            if (RC.isDown() && isOverObj(tip, apple_translation) && !currentlyMovingAnApple){
              currentlyMovingAnApple = true;
               anAppleHasBeenPicked = true;
              a["isPicked"] = true;
              a["isMoving"] = true;
            } 
            // If the apple was moving but we now release
            if (RC.release() &&   a["isMoving"]){
              currentlyMovingAnApple = false;
              a["isMoving"] = false;
              a["newPos"] =  current_RC_position;
            }
        }
...
function onDraw(t, projMat, viewMat, state, eyeIdx) {
    ...
    let drawShape = (shape, color, texture, textureScale) => {
       ...
       gl.drawArrays((shape == cube || shape == tree || shape == apple ) ? gl.TRIANGLES : gl.TRIANGLE_STRIP, 0, shape.length / VERTEX_SIZE);
       prev_shape = shape;
    }
    ...
    m.identity();
      ...
      m.save();
        if (anAppleHasBeenPicked){       
          for (var i = 0; i < Object.keys(apples).length; i++){
            //Draw apples that have been modified
                var thisApple = apples[i];
                if (thisApple["isPicked"] == true){
                    m.save();
                    if (thisApple["isMoving"]){

    
                      var t = current_RC_position;
                      m.translate(t[0],t[1],t[2]); 
                      m.scale(.3,.3,.3);
                      m.scale(.15,.15,.15);
                      drawShape(apple, [1,1,1],0);

                    }  else {
                        var t = thisApple["newPos"];
                        m.translate(t[0],t[1],t[2]); 
                        m.scale(.3,.3,.3);
                        m.scale(.15,.15,.15);
                        if (thisApple["hover"] == true){
                          drawShape(apple, [1,.5,.5],0);
                        } else {
                          drawShape(apple, [1,1,1],0);
                        }
                    }
    ...
    m.save();
      ...
      m.save();
        ...
        m.scale(12,.01,12);
        m.rotateX(Math.PI/2);
        drawShape(cylinder, [1,1,1],1);
      m.restore();

      // Draw random apples that surround trees
            ...
            m.scale(.3,.3,.3);
              ...
              drawShape(tree, [1,1,1]);
                  ...
                  for (var i =0;i<4;i++){
                    ...
                    m.scale(.2,.2,.2);
                    m.rotateY(state.time-i);
                    m.rotateX(state.time-i);

                    // console.log("apple");

              
                      if (!(appleNumber in apples)){
                        apples[appleNumber] = {
                          "originalPos": m.value(),
                          "isPicked": false,
                          "newPos":null,
                      ...
                      if (apples[appleNumber]["isPicked"] == false) {
                        //Only draw shape and save its original position if it has not been picked
                        apples[appleNumber]["originalPos"] =  m.value();
                        if (apples[appleNumber]["hover"] == true){
                          drawShape(apple, [1,.5,.5], 0);
                        } else {
                          drawShape(apple, [1,1,1],0);
                        }
